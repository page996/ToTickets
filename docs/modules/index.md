...
| `M9-console-contract-ui` | `apps/console/src` + `src-tauri` | active，loopback 浏览器回归有历史证据 | partial | `CP-20260902-module-governance-baseline :: M9` | REST/WS client、Tauri IPC runtime config | Console 9 files/81 tests；R2 浏览器证据 | Tauri 原生窗口人工验收；host/deployment UI |
...
| `CON-C4-event-stream` | `apps/console/src/api/event-stream.ts` | active | `event-stream.sync.v1`、CloudEvents | `event-stream.spec.ts` | `CP-20260902-module-governance-baseline :: CON-C4` | formal fake port、跨进程恢复证据 |
...
| `CON-C6-ui-features` | `apps/console/src/App.tsx`、`components/*`、`styles.css` | active，浏览器回归有历史证据 | UI state/actions | component specs、R2 browser evidence | `CP-20260902-module-governance-baseline :: CON-C6` | App 根状态递归拆分；Tauri 人工验收 |
